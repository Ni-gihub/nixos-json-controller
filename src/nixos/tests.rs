...
use super::discovery::inspection::NixosConfiguration;
...
/// 現在のNixOS設定リポジトリを使ってDiscovery stateを準備する。
///
/// 本番の `nxc discover` と同じく、保存時に
/// flake.nix / flake.lock のmetadataを取得する。
///
...
fn generate_empty_package_module() {
    let module = NixModule { packages: vec![] };
